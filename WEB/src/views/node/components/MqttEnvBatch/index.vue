...
import { readMqttPortsFromTags, SETUP_COPY } from '../../utils/constants';
...
defineOptions({ name: 'MqttEnvBatch' });
...
async function runCheck() {
  ...
  try {
    ...
    for (const node of selectedNodes.value) {
      if (!node.id) continue;
      try {
        const data = await checkMqttStackBySsh(node.id);
        results.push({
          nodeId: node.id,
          nodeName: node.name,
          host: node.host,

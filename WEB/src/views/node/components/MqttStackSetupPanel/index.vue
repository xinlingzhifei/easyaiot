...
import {
  checkMqttStackBySsh,
  checkMqttPortsBySsh,
  deployMqttStackBySsh,
  removeMqttContainerBySsh,
  removeMqttImageBySsh,
  stopMqttServiceBySsh,
  type MqttStackCheckResult as MqttStackCheckData,
  type PortCheckResult as PortCheckData,
} from '@/api/device/node';
import {
  buildMqttStackManualContent,
  getMqttStackGuideState,
  MQTT_STACK_DEPLOY_PENDING_STEPS,
  SETUP_COPY,
  SETUP_FORM_LABEL_COL,
  SETUP_FORM_WRAPPER_COL,
  type MqttStackScriptParams,
...
const checkResult = ref<MqttStackCheckData | null>(null);
const portChecking = ref(false);
const portCheckResult = ref<PortCheckData | null>(null);
const mqttOpLoading = ref<'stop-emqx' | 'remove-container' | 'remove-image' | null>(null);
const mqttOpResult = ref<DeployResultState | null>(null);
const hasAutoChecked = ref(false);

const guide = computed(() => getMqttStackGuideState(props.formValues));
...
const autoDisabledReason = computed(() => {
  if (!guide.value.isReady) return '请先在节点配置中填写主机地址与 MQTT 端口';
...
const manualContent = computed(() => {
  ...
  return buildMqttStackManualContent(props.formValues || {});
});
...
async function handleCheckDeploy() {
  ...
  try {
    checkResult.value = await checkMqttStackBySsh(nodeId);
  } catch (e: unknown) {
...
async function handleCheckPorts() {
  ...
  try {
    portCheckResult.value = await checkMqttPortsBySsh(nodeId);
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : '检测请求失败';
...
async function runMqttOp(
  op: 'stop-emqx' | 'remove-container' | 'remove-image',
  request: () => Promise<{ success?: boolean; message?: string; steps?: DeployResultState['steps'] }>,
  successMsg: string,
) {
  const nodeId = props.formValues?.nodeId;
  if (!nodeId || mqttOpLoading.value) return;
  mqttOpLoading.value = op;
  mqttOpResult.value = null;
  try {
    ...
    if (data?.success) {
      ...
      createMessage.error(mqttOpResult.value.message || '操作失败');
    }
  } catch (e: unknown) {
    ...
    mqttOpLoading.value = null;
  }
}

function handleStopEmqx() {
  const nodeId = props.formValues?.nodeId;
  if (!nodeId) return;
  runMqttOp('stop-emqx', () => stopMqttServiceBySsh(nodeId), 'EMQX 已停止');
}

function handleRemoveContainer() {
  const nodeId = props.formValues?.nodeId;
  if (!nodeId) return;
  runMqttOp('remove-container', () => removeMqttContainerBySsh(nodeId), 'EMQX 容器已删除');
}

function handleRemoveImage() {
  const nodeId = props.formValues?.nodeId;
  if (!nodeId) return;
  runMqttOp('remove-image', () => removeMqttImageBySsh(nodeId), 'EMQX 镜像已删除');
}
...
async function handleAutoDeploy() {
  ...
  try {
    const data = await deployMqttStackBySsh(nodeId, { signal: deployAbort.value.signal });
    deployResult.value = {
      success: !!data?.success,
      message: resolveDeployMessage(data || {}),
      steps: data?.steps || [],
...
<template>
  <SetupStepShell v-if="guide.isMqttRole">
    <template v-if="!guide.isReady" #intro>
      <Alert type="warning" show-icon :message="`${SETUP_COPY.mqttService}配置不完整`">
    ...
    <CollapseContainer v-if="guide.isReady" :title="SETUP_COPY.deployConfig" :can-expan="false">
      ...
      >
        ...
        <FormItem :label="SETUP_COPY.mqttPortCheck">
          <Space wrap>
            <Button
          ...
          <div v-else class="form-hint">检测 MQTT / SSL / WS / Dashboard 端口是否被占用</div>
        </FormItem>

        <FormItem :label="SETUP_COPY.mqttDeployCheck">
          <Space wrap>
            <Button :loading="checking" :disabled="!canCheckDeploy || deploying" @click="handleCheckDeploy">
              {{ SETUP_COPY.checkMqttDeployBtn }}
            </Button>
          </Space>
          <div v-if="!canCheckDeploy" class="form-hint">{{ autoDisabledReason || '请先保存节点并配置 SSH' }}</div>
          <MqttStackCheckResult v-else-if="checkResult" :result="checkResult" @close="checkResult = null" />
          <div v-else class="form-hint">通过 SSH 探测目标机 Docker、EMQX 运行状态，不会修改目标机</div>
        </FormItem>
        ...
        <FormItem v-if="deployMode === 'auto'" :label="SETUP_COPY.mqttOps">
          <Space wrap>
            <Button
              :loading="mqttOpLoading === 'stop-emqx'"
              :disabled="!canCheckDeploy || deploying || !!mqttOpLoading"
              @click="handleStopEmqx"
            >
              {{ SETUP_COPY.stopEmqxBtn }}
            </Button>
            <Button
              danger
              :loading="mqttOpLoading === 'remove-container'"
              :disabled="!canCheckDeploy || deploying || !!mqttOpLoading"
            ...
            <Button
              danger
              :loading="mqttOpLoading === 'remove-image'"
              :disabled="!canCheckDeploy || deploying || !!mqttOpLoading"
    ...
    <CollapseContainer
      v-if="deployMode === 'auto' && guide.isReady && (deploying || deployResult || mqttOpLoading || mqttOpResult)"
      :title="SETUP_COPY.deployProgress"
      :can-expan="false"
    >
      <DeployProgressPanel
        :loading="deploying || !!mqttOpLoading"
        :result="deployResult || mqttOpResult"
        :pending-steps="[...MQTT_STACK_DEPLOY_PENDING_STEPS]"
        :show-stop="deploying"

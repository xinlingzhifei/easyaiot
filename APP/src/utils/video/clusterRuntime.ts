...
export function resolveRuntimeHost(record?: ClusterRuntimeRecord | null): string {
  if (!record)
    return ''
  const raw = record.service_server_ip || record.server_ip || record.assigned_node_host || ''
  return String(raw).split(',')[0]?.trim() || ''
}

export function resolveRuntimeNodeId(record?: ClusterRuntimeRecord | null): string {
  if (!record)
    return ''
...
export function resolveRuntimePid(record?: ClusterRuntimeRecord | null): string {
  if (!record)
    return ''
...
export function formatClusterRuntime(record?: ClusterRuntimeRecord | null): string {
  if (!record)
    return '--'

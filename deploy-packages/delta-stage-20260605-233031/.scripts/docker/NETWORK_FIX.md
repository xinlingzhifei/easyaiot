...
当宿主机IP地址发生变化后，Redis容器（或其他容器）可能无法加入 `yfeieye-network` 网络。
...
```bash
cd /projects/yfeieye/.scripts/docker
./fix_network.sh
```
...
3. **定期检查**：定期运行 `docker network inspect yfeieye-network` 检查网络状态
...
# 测试网络连通性
docker run --rm --network yfeieye-network alpine:latest ping -c 1 8.8.8.8

# 查看连接到网络的容器
docker network inspect yfeieye-network --format '{{range .Containers}}{{.Name}} {{end}}'
```
...
```
Error: network yfeieye-network not found
```
**解决方法**：运行修复脚本或手动创建网络

### 错误2：network is in use
```
Error: network yfeieye-network is in use
```
**解决方法**：先停止所有使用该网络的容器，然后删除网络

### 错误3：container cannot connect to network
```
Error: container cannot connect to network yfeieye-network
```

...
def _strip_rtsp_transport_query(source_url: str) -> tuple[str, Optional[str]]:
    ...
    支持: yfeieye_rtsp_transport / rtsp_transport / iot_rtsp_transport = tcp|udp
    """
    try:
        ...
        transport = None
        for key in ("yfeieye_rtsp_transport", "rtsp_transport", "iot_rtsp_transport"):
...
def update_device(device_id):
    ...
    try:
        ...
        if 'manufacturer' in data:
            manufacturer = data.get('manufacturer', '').strip() if data.get('manufacturer') else ''
            if not manufacturer:
                data['manufacturer'] = 'yFeiEye'
        if 'model' in data:
            model = data.get('model', '').strip() if data.get('model') else ''
            if not model:
                data['model'] = 'Camera-yFeiEye'

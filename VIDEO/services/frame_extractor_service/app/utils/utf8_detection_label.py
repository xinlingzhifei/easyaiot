...
def get_overlay_font(font_size: int = _DEFAULT_FONT_SIZE) -> Optional[ImageFont.FreeTypeFont]:
    ...
    for candidate in _font_candidates():
        ...
        try:
            ...
            logger.warning("抽帧服务中文字体加载失败 %s: %s", candidate, exc)
    return None
...
def _warn_missing_cjk_font_once() -> None:
    logger.warning(
        "未找到抽帧服务中文字体，非 ASCII 类别名将回退 OpenCV；"
...
def _draw_ascii_label(
    ...
    if background_bgr is not None:
        cv2.rectangle(
            ...
            (
                min(frame.shape[1] - 1, text_x + text_width + 2),
                min(frame.shape[0] - 1, text_y + baseline + 2),
            ),
...
def draw_detection_label(
    ...
    *,
    ...
    drawer.text(
        (text_left - roi_left - bbox[0], text_top - roi_top - bbox[1]),
        normalized_text,
        font=font,
...
__all__ = ["draw_detection_label", "get_overlay_font"]

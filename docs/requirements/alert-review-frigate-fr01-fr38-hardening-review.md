...
| --- | --- | --- | --- |
| FR-01 | Alert review item aggregation | DEVICE tests cover clue ingest, merge windows, and tenant-scoped ingest identity; all 18 mapped supervision/review entities declare `@TableId` plus their matching PostgreSQL `@KeySequence`, required because the release PostgreSQL profile resolves the global MyBatis ID mode to `INPUT`; `PG1` covers historical backfill and duplicate races. VIDEO production `/video/alert/hook` requires an exact-byte service-HMAC `alert_ingest` context with tenant/camera identity, and realtime/snapshot/patrol algorithm senders use the signed request builder; unsigned ingest is limited to an explicit development/test switch. | Rerun `PG1`, execute deployed ingest to prove IDs come from the 18 declared sequences rather than `NULL`, then prove real algorithm-to-VIDEO signed ingest, nonce/replay rejection, camera scope, and key rotation. |
...
- `PG1`: `node .scripts/alert-review-postgres-migration-smoke.mjs --container=<postgres-container>` or `node .scripts/alert-review-postgres-migration-smoke.mjs --database-url=postgresql://.../postgres`; direct URL mode keeps the password out of `psql` argv. The current plan applies `V20260701` through `V20260713_3` in `MIGRATION_FILES` order, including `V20260708_10` SMALLINT deletion alignment, `V20260709` active scheduler jobs, `V20260710` persistent export queue claims/idempotency, `V20260711` `media:manage`, `V20260712` semantic-trigger permission/terminal indexes, `V20260713` semantic-index generation/claim/lease/retry fields, `V20260713_2` evidence record start, and `V20260713_3` `supervision:event:create`, in addition to the earlier ingest, ReviewData, ReviewSegment, media-audit, report, and outbox assertions.
- `Smoke`: `POST /system/supervision/alert-review/integration-smoke` against release DEVICE + real VIDEO recordings.
- `LiveDevice`: `node .scripts/alert-review-device-integration-smoke.mjs --device-base-url=... --token=... --tenant-id=... --operator-user-id=... --alert-time="YYYY-MM-DDTHH:mm:ss" [--playback-allowed-camera-ids=... --playback-denied-camera-ids=...]`; requires a deployed DEVICE service and fails unless the endpoint returns `passed`, `manifestValid=true`, `videoExportRequested=true`, and all ingest / coverage / case / export / verify / download-audit checkpoints; when playback camera params are supplied, it also requires playback URL allow/deny decisions to produce `playback_url_granted` and `playback_url_denied`.
...
| --- | --- | --- | --- | --- |
| FR-01 | DEVICE clue ingest plus VIDEO `POST /video/alert/hook` | ingest item/identity/evidence tables and their PostgreSQL ID sequences; VIDEO `media_authorization_service.py` signed `alert_ingest` request | DEVICE ingest/idempotency tests, `SupervisionPersistenceIdContractTest`; `test_alert_ingest_request_builder_round_trips_through_service_hmac`, `test_alert_ingest_sender_posts_the_signed_bytes_not_a_reserialized_json_body`, unsigned-production rejection tests | `J1`, `V2`, `PG1`; deployed sequence-backed ingest plus real algorithm caller/HMAC rotation smoke required |
...
| FR-05 | `POST /items/{id}/to-event`, `POST /system/supervision/events/from-alert`, `supervisionAlertReviewEventReconcileJob` | `system_supervision_event`, `system_supervision_alert_review_item.review_data`, `V20260709__alert_review_scheduler_activation.sql`, `V20260713_3__supervision_event_create_permission.sql` | `convertReviewItemToSupervisionEventUsesReviewItemAsIdempotentSource`, `createFromAlertCanPersistSyntheticEventWithoutDispatchingTasks`, controller permission tests, `eventReconcileJobPersistsReverseEventProjectionOutsideListQuery`, `SupervisionAlertReviewRuntimeSchedulingTest` | `J1`, `J2`, `PG1`, `Smoke`; grant intended roles, prove normal dispatch and smoke zero-task event |
...
| DEVICE schema and migration | Existing `V20260701`-`V20260708_9`, plus `V20260708_10__alert_review_deleted_smallint.sql`, `V20260709__alert_review_scheduler_activation.sql`, `V20260710__alert_review_export_queue.sql`, `V20260711__alert_review_media_manage_permission.sql`, `V20260712__alert_review_semantic_trigger_confirmation.sql`, `V20260713__alert_review_semantic_index_claim.sql`, `V20260713_2__alert_review_evidence_record_start.sql`, `V20260713_3__supervision_event_create_permission.sql`, and schema/PG smoke tests | Tracked by the release package verifier | Package all forward migrations together; run `PG1` on the release shape before deploy and assign the new event-create permission to intended roles |

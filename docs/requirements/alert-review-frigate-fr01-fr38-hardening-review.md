...
| VIDEO deployment | `/opt/yfeieye-source/current` and the `/app` bind mount resolve to `/opt/yfeieye-source/releases/20260715-fc972b4e-fr-replayable`; final `record_export_service.py` SHA-256 `dbbf3493f763f5e4ec469911676ffde2e168562bdafd6ec93ef9d377b0c13e17`; `video-service` healthy with restart count 0. |
...
- VIDEO `/app` is mounted from `/opt/yfeieye-source/releases/20260715-fc972b4e-fr-replayable/VIDEO`, not from the older 4ac release. The release contains audit-lock serialization, Shanghai queue-clock handling, cleared retry state, and original-URI replay recovery.

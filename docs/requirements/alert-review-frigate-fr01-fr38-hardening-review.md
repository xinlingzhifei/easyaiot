...
| FR-32 | Production smoke | Integration smoke now covers ingest -> coverage -> case -> export -> verify -> download audit, with checkpoints and evidence audit trail regression; `LiveDevice` executes the deployed DEVICE endpoint and rejects missing checkpoint / invalid manifest / non-video-export responses, and can opt in to deployed playback-url allow/deny probes; `LiveVideo` covers the external VIDEO alert-record, coverage, record-base, export, export download-ready, download URL reachability, and manifest v2 reproducibility fields without starting mocks; `LivePlayer` covers deployed workbench click-to-player seek payloads without starting mocks; `ProdSmoke` runs all three in order as one release gate and requires playback allow/deny camera params before forwarding them into `LiveDevice`. | Run `ProdSmoke` against real recordings, playback allow/deny camera scopes, and the release environment. |
...
- `ProdSmoke`: `node .scripts/alert-review-production-smoke.mjs --device-base-url=... --token=... --operator-user-id=... --device-alert-time=... --device-playback-allowed-camera-ids=... --device-playback-denied-camera-ids=... --video-alert-record-query-url=... --video-record-coverage-query-url=... --video-record-base-url=... --video-record-export-url=... --video-device-id=... --video-alert-time=... --player-workbench-url=... --player-review-row-text=... --player-expected-seek-time=... --player-expected-record-path-contains=... --player-expected-offset-seconds=...`; runs `LiveDevice`, `LiveVideo`, and `LivePlayer` sequentially and fails the release on the first failed real-service smoke.

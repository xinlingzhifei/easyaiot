...
| FR-32 | Production smoke | Integration smoke now covers ingest -> coverage -> case -> export -> verify -> download audit, with checkpoints and evidence audit trail regression; `LiveVideo` covers the external VIDEO alert-record, coverage, record-base, and export endpoints without starting mocks. | Make deploy smoke mandatory against real VIDEO recordings and release environment. |
...
| FR-36 | Frontend E2E | Workbench E2E now has explicit `all`, `contract`, and `dev-api-mock` modes, package-script aliases, invalid-mode runner coverage, exact player seek payload assertions, playback offset contract coverage, and region-rule save assertions. | Keep full `vue-tsc` and real release API/player smoke as separate release blockers. |
...
| FR-13 | `GET /items/{id}/detail-stream`, workbench seek action | `reviewData.reviewSegment`, `system_supervision_alert_review_segment`, `DialogPlayer` playback payload | `reviewDetailStreamExpandsObjectsDetectionsAndSeekTimes`, workbench `alert-review-detail-seek`, `alert-review-coverage-seek`, and `alert-review-case-timeline-seek` payload assertions, `alert-review-playback-contract.test.mjs` | `J1`, `W1`, `W3`; real player seek smoke required |
...
| FR-32 | `POST /integration-smoke`, `alert-review-video-live-smoke.mjs` | smoke result payload, evidence audit chain, real VIDEO endpoint checkpoints | `integrationSmokeCoversReviewRecordCaseExportAndManifestVerification`, workbench mock checkpoint `evidence_download_audited`, `alert-review-video-live-smoke.test.mjs` | `J1`, `W1`, `LiveVideo`, `Smoke` |
...
| FR-36 | workbench browser contract harness | `WEB/scripts/fixtures/alert-review-workbench-e2e/*`, `WEB/scripts/alert-review-workbench-e2e-check.mjs`, `WEB/scripts/alert-review-workbench-e2e-check.test.mjs`, `WEB/scripts/alert-review-playback-contract.test.mjs` | `alert-review-workbench-e2e-check.mjs --mode=contract`, `--mode=dev-api-mock`, invalid-mode runner test, playback offset contract test | `W1`, `W3`; real release API smoke still required |
...
- Release package verifier self-test passed:
  `node .scripts/verify-alert-review-release-package.test.mjs`
  Result: `alert review release package verifier tests OK`; the verifier now tracks `V20260704`, PG1 PostgreSQL smoke tooling, LiveVideo smoke tooling, the workbench runner test, playback contract test, and `WEB/src/utils/withInstall.ts`.
...
- Workbench-to-player seek handoff now preserves `seek_time` plus `playback_offset_seconds` and native mp4 VOD can consume the offset; the remaining release risk is a real browser/player smoke against a deployed workbench and real recording URL.

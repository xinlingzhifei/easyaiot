...
- FR-07/FR-22 VIDEO availability gap reason catalog passed:
  ...
  `node .scripts/verify-alert-review-release-package.test.mjs`
  Result: RED first failed because VIDEO returned `gap_reason_category=unknown` for `file_missing`, `probe_failed`, `disk_full`, and `cache_flush_failed`; GREEN reruns passed after `_normalize_gap_reason` classified those standard reasons as `filesystem`, `probe`, `storage`, and `cache`, with `Pkg` blocking removal of the VIDEO record gap reason catalog anchors.

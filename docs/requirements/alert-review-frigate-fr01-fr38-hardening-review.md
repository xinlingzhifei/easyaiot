...
- `PG1`: `node .scripts/alert-review-postgres-migration-smoke.mjs --container=<postgres-container>`; applies `V20260702`, `V20260704`, `V20260705`, `V20260706`, `V20260707`, and `V20260708` to a temporary database and verifies FR-01 ingest identity, FR-03 review status/version stale writes and concurrent races, FR-19 ReviewData backfill, FR-20 ReviewSegment overlap and status-transition constraints, FR-23 media permission seeds, and FR-23 item-level media audit lookup against PostgreSQL.
...
- Runtime patrol, runtime outbox, and event reverse reconciliation have schedulable JobHandlers; event rollback conflict policy is code-covered, and production release must configure scheduler triggers, final alert/notification sink, and a real rollback/rework event smoke.

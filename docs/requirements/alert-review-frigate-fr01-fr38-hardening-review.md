...
| FR-32 | Production smoke | Integration smoke now covers ingest -> coverage -> case -> export -> verify -> download audit, with checkpoints and evidence audit trail regression; `LiveVideo` covers the external VIDEO alert-record, coverage, record-base, export, export download-ready, download URL reachability, and manifest v2 reproducibility fields without starting mocks; `LivePlayer` covers deployed workbench click-to-player seek payloads without starting mocks. | Make deploy smoke mandatory against real VIDEO recordings and release environment. |
...
| FR-32 | `POST /integration-smoke`, `alert-review-video-live-smoke.mjs`, `alert-review-player-live-smoke.mjs` | smoke result payload, evidence audit chain, real VIDEO endpoint checkpoints, export download readiness, download URL reachability, manifest v2 reproducibility fields, deployed player seek payload | `integrationSmokeCoversReviewRecordCaseExportAndManifestVerification`, workbench mock checkpoint `evidence_download_audited`, `alert-review-video-live-smoke.test.mjs`, `alert-review-player-live-smoke.test.mjs` | `J1`, `W1`, `LiveVideo`, `LivePlayer`, `Smoke` |

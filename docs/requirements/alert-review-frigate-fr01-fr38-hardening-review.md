...
- FR-21/FR-32 LiveVideo local file path evidence safety passed after the RED failure showed VIDEO could return only `file_path=/var/lib/...` or `record_uri=C:\...` and satisfy the release smoke as playable evidence:
  `node .scripts/alert-review-video-live-smoke.test.mjs`
  `node .scripts/verify-alert-review-release-package.test.mjs`
  Result: RED first failed with `Missing expected rejection`, then the package fixture showed `Pkg` would not block removing the file-path / absolute-path guard; GREEN reruns passed after `LiveVideo` preserved the record URI source, rejected `file_path` and absolute local filesystem evidence in release mode, and `Pkg` scanned for those guards.

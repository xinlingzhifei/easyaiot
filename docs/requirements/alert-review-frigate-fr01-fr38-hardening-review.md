...
- FR-21/FR-32 LiveVideo local file path evidence safety passed after the RED failure showed VIDEO could return only `file_path=/var/lib/...` and satisfy the release smoke as playable evidence:
  `node .scripts/alert-review-video-live-smoke.test.mjs`
  `node .scripts/verify-alert-review-release-package.test.mjs`
  Result: RED first failed with `Missing expected rejection`, then the package fixture showed `Pkg` would not block removing the file-path guard; GREEN reruns passed after `LiveVideo` preserved the record URI source, rejected `file_path` evidence in release mode, and `Pkg` scanned for that source guard.

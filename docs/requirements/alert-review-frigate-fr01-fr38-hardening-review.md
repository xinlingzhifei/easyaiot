...
- `ProdSmoke`: `node .scripts/alert-review-production-smoke.mjs --device-base-url=... --token=... --operator-user-id=... --device-alert-time=... --video-alert-record-query-url=... --video-record-coverage-query-url=... --video-record-base-url=... --video-record-export-url=... --video-device-id=... --video-alert-time=... --player-workbench-url=... --player-review-row-text=... --player-expected-seek-time=... --player-expected-record-path-contains=... --player-expected-offset-seconds=...`; runs `LiveDevice`, `LiveVideo`, and `LivePlayer` sequentially and fails the release on the first failed real-service smoke.
...
| FR-32 | `POST /integration-smoke`, `alert-review-device-integration-smoke.mjs`, `alert-review-video-live-smoke.mjs`, `alert-review-player-live-smoke.mjs`, `alert-review-production-smoke.mjs` | smoke result payload, evidence audit chain, deployed DEVICE checkpoint assertion, real VIDEO endpoint checkpoints, export download readiness, download URL reachability, manifest v2 reproducibility fields, deployed player seek payload, one-command production smoke orchestration | `integrationSmokeCoversReviewRecordCaseExportAndManifestVerification`, workbench mock checkpoint `evidence_download_audited`, `alert-review-device-integration-smoke.test.mjs`, `alert-review-video-live-smoke.test.mjs`, `alert-review-player-live-smoke.test.mjs`, `alert-review-production-smoke.test.mjs` | `J1`, `W1`, `ProdSmoke`, `Smoke` |
...
- Production smoke with real VIDEO URLs:
  `node .scripts/alert-review-production-smoke.mjs --device-base-url=... --token=... --operator-user-id=... --device-alert-time=... --video-alert-record-query-url=... --video-record-coverage-query-url=... --video-record-base-url=... --video-record-export-url=... --video-device-id=... --video-alert-time=... --player-workbench-url=... --player-review-row-text=... --player-expected-seek-time=... --player-expected-record-path-contains=... --player-expected-offset-seconds=...`

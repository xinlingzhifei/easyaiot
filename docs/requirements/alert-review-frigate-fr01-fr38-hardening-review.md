...
| FR-36 | workbench browser contract harness and release player smoke | `WEB/scripts/fixtures/alert-review-workbench-e2e/*`, `WEB/scripts/alert-review-workbench-e2e-check.mjs`, `WEB/scripts/alert-review-workbench-e2e-check.test.mjs`, `WEB/scripts/alert-review-playback-contract.test.mjs`, `.scripts/alert-review-player-live-smoke.mjs` | `alert-review-workbench-e2e-check.mjs --mode=contract`, `--mode=dev-api-mock`, `--mode=dev-api-real-drawer`, invalid-mode runner test, playback offset contract test, player live smoke self-test | `W1`, `W3`, `LivePlayer`; real release API smoke still required |
...
- Runtime patrol, runtime outbox, event reverse reconciliation, and semantic indexing have paused scheduler seeds; event rollback conflict policy is code-covered, and production release must enable/tune scheduler triggers, wire the final alert/notification sink, and run real rollback/rework plus backlog smokes.

...
| FR-15 | Rule safe apply | Tests cover accepted-before-applied, shadow evaluation, rollback, and controller permission annotations on rule suggestion status, rollback, and replay endpoints. | Add UI permission gating and release smoke for operator roles. |
...
| FR-15 | `POST /items/{id}/rule-suggestion/status`, `/revert`, `/rules/replay` | `system_supervision_alert_review_rule`, rule suggestion payload, controller permission annotations | `ruleSuggestionGovernanceEndpointsDeclareApprovalPermissions`, `ruleSuggestionSafeApplyRequiresApprovalAndStoresShadowEvaluation`, `falsePositiveRuleSuggestionAppliesRuleConfigOnlyAfterApprovalAndCanRollback` | `J1` |

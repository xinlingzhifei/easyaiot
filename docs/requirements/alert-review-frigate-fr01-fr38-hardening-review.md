...
| FR-17 | Evidence-chain audit | Tests cover manifest verification, media access allow/deny audit, bound events, downloads, hash chaining, and audit reverse lookup metadata for `reviewCaseId`, `reviewItemIds`, `eventIds`, and `exportJobNo`; media access reads now enter the evidence audit trail with operator, camera, material URI, decision, denied reasons, and returned coverage record URIs; production smoke evidence now preserves DEVICE child summary identifiers and an `auditChain` object with `action=export_downloaded`, `reviewCaseId`, `reviewItemIds`, `eventIds`, and `exportJobNo`, and each player smoke summary plus LiveVideo export/signature/verifier/storage-lifecycle summary now carries traceability metadata without recording raw stdout, signed player URL / recordPath / export download / manifest query strings, LiveDevice playback URLs/debug tokens, signer/signature debug tokens, verifier debug tokens, storage lifecycle URLs/tokens, or URL-valued step command query/hash secrets. | Run real playback/snapshot/download/manifest allow-deny audit smoke against deployed endpoints. |
...
- FR-17/FR-32 production smoke LiveVideo exportResult sanitizer passed after the RED failure showed signed export `downloadUrl` / `manifestUrl` values could leak query/hash secrets into the sanitized evidence report:
  `node .scripts/alert-review-production-smoke.test.mjs`
  `node .scripts/verify-alert-review-release-package.test.mjs`
  Result: RED first failed with a deep-equal mismatch because `exportResult.downloadUrl` and `manifestUrl` still contained `export-download-secret` / `export-manifest-secret`; a package RED then showed `Pkg` would not block removing the sanitizer. GREEN reruns passed after `ProdSmoke` summarizes LiveVideo export results through a URL-secret stripping helper and `Pkg` scans for both export URL fields.

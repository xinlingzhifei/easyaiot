...
- FR-21/FR-32 LiveVideo local file path evidence safety passed after the RED failure showed VIDEO could return only `file_path=/var/lib/...`, `record_uri=C:\...`, or `record_uri=//localhost/...` and satisfy the release smoke as playable evidence:
  `node .scripts/alert-review-video-live-smoke.test.mjs`
  `node .scripts/verify-alert-review-release-package.test.mjs`
  Result: RED first failed with `Missing expected rejection`, then the package fixture showed `Pkg` would not block removing the file-path / absolute-path / protocol-relative guard; GREEN reruns passed after `LiveVideo` preserved the record URI source, rejected `file_path`, absolute local filesystem evidence, and protocol-relative local/mock URLs in release mode, and `Pkg` scanned for those guards.

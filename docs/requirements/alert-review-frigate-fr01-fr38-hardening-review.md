...
| FR-17 | Evidence-chain audit | Tests cover manifest verification, media access allow/deny audit, bound events, downloads, hash chaining, and audit reverse lookup metadata for `reviewCaseId`, `reviewItemIds`, `eventIds`, and `exportJobNo`; media access reads now enter the evidence audit trail with operator, camera, material URI, decision, denied reasons, and returned coverage record URIs; production smoke evidence now preserves DEVICE child summary identifiers and an `auditChain` object with `action=export_downloaded`, `reviewCaseId`, `reviewItemIds`, `eventIds`, and `exportJobNo`, and each player smoke summary plus LiveVideo export/verifier summary now carries traceability metadata without recording raw stdout, signed player URL / recordPath / export download / manifest query strings, LiveDevice playback URLs/debug tokens, verifier debug tokens, or URL-valued step command query/hash secrets. | Run real playback/snapshot/download/manifest allow-deny audit smoke against deployed endpoints. |

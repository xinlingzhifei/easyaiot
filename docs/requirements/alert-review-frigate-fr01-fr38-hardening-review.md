...
| FR-23 | Permission and audit enforcement | Media access audit API exists; evidence export, download, item timeline, detail stream, record coverage, case timeline, manifest verification, and evidence package verification now resolve a server-side camera scope through `ReviewCameraPermissionResolver`; package verification also binds audit operator identity to the logged-in user instead of request `operatorUserId`; coverage reads preserve the same media-in-case guard by attaching returned record URIs only after case/camera scope precheck and before auditing; workbench playback from detail stream, unified timeline coverage, and coverage list now audits both pre-case item-level opens and active-case opens before emitting `viewVideo`, while case timeline playback audits the active review case; request `allowedCameraIds` can only narrow that scope, forged camera lists are denied and audited, `yfeieye.review.camera-permission` can fail closed, action-specific media permissions can be bound to the real `PermissionService`; `V20260706` seeds release `system_menu` button permissions for playback/export/download/manifest media actions; `V20260707` allows item-level media audit rows before a review case exists; and review items now persist `tenant_id` with tenant-aware workbench/merge indexes. | Assign the seeded media permissions to release roles, replace configured camera maps with the final tenant/user/camera permission source, and connect the same guard to the concrete backend playback URL endpoint once it is connected. |
...
| FR-32 | Production smoke | Integration smoke now covers ingest -> coverage -> case -> export -> verify -> download audit, with checkpoints and evidence audit trail regression; `LiveDevice` executes the deployed DEVICE endpoint and rejects missing checkpoint / invalid manifest / non-video-export responses; `LiveVideo` covers the external VIDEO alert-record, coverage, record-base, export, export download-ready, download URL reachability, and manifest v2 reproducibility fields without starting mocks; `LivePlayer` covers deployed workbench click-to-player seek payloads without starting mocks. | Make `LiveDevice`, `LiveVideo`, and `LivePlayer` mandatory against real recordings and the release environment. |
...
- `LiveDevice`: `node .scripts/alert-review-device-integration-smoke.mjs --device-base-url=... --token=... --operator-user-id=... --alert-time="YYYY-MM-DDTHH:mm:ss"`; requires a deployed DEVICE service and fails unless the endpoint returns `passed`, `manifestValid=true`, `videoExportRequested=true`, and all ingest / coverage / case / export / verify / download-audit checkpoints.
- `LiveVideo`: `node .scripts/alert-review-video-live-smoke.mjs --alert-record-query-url=... --record-coverage-query-url=... --record-base-url=... --record-export-url=... --device-id=... --alert-time="YYYY-MM-DD HH:mm:ss"`; requires a real VIDEO service and real recording metadata, and fails unless export status exposes a reachable `download_url` plus a `manifest_url` whose manifest v2 includes ffmpeg command hash, source segment hashes, clip params, concat order, and output hashes.
...
| FR-23 | Media access guard on timeline, coverage, export, verify, download | audit entries in case audit store, item-level media audit lookup, permission resolver config, `PermissionService` action gate, `V20260706__alert_review_media_permissions.sql`, `V20260707__alert_review_item_media_audit.sql`, workbench pre-case and active-case media playback guard | `evidenceExportRejectsUnauthorizedCameraMediaAndAuditsDenial`, `recordCoverageRejectsUnauthorizedCameraMediaAndAuditsDenial`, `allowedRecordCoverageReadAuditsReturnedRecordUrisInEvidenceTrail`, `manifestVerificationRejectsUnauthorizedCameraMediaAndAuditsDenial`, `evidencePackageVerificationRejectsUnauthorizedCameraMediaAndAuditsDenial`, `requestedCameraScopeCannotExpandServiceSideCameraPermission`, `evidencePackageVerificationUsesLoginUserInsteadOfRequestOperator`, `configuredScopesRequireRealActionPermissionWhenPermissionServiceIsPresent`, `actionPermissionKeysAreNormalizedBeforeGateLookup`, `alertReviewMediaPermissionMigrationSeedsMenuPermissions`, `preCaseMediaAccessAuditRecordsAllowDenyAndCanBeListedByReviewItem`, `alertReviewItemMediaAuditMigrationAllowsPreCaseAuditRows`, `alert-review-postgres-migration-smoke.mjs`, workbench dev-api-mock pre-case and active-case media audit ordering checks | `J1`, `PG1`, `W1`; real tenant/camera resolver smoke required |
...
| FR-32 | `POST /integration-smoke`, `alert-review-device-integration-smoke.mjs`, `alert-review-video-live-smoke.mjs`, `alert-review-player-live-smoke.mjs` | smoke result payload, evidence audit chain, deployed DEVICE checkpoint assertion, real VIDEO endpoint checkpoints, export download readiness, download URL reachability, manifest v2 reproducibility fields, deployed player seek payload | `integrationSmokeCoversReviewRecordCaseExportAndManifestVerification`, workbench mock checkpoint `evidence_download_audited`, `alert-review-device-integration-smoke.test.mjs`, `alert-review-video-live-smoke.test.mjs`, `alert-review-player-live-smoke.test.mjs` | `J1`, `W1`, `LiveDevice`, `LiveVideo`, `LivePlayer`, `Smoke` |
...
| Release gate tooling | `.scripts/verify-alert-review-release-package.mjs`, `.scripts/verify-alert-review-release-package.test.mjs`, `.scripts/record-export-manifest-verifier.mjs`, `.scripts/alert-review-postgres-migration-smoke.mjs`, `.scripts/alert-review-postgres-migration-smoke.test.mjs`, `.scripts/alert-review-device-integration-smoke.mjs`, `.scripts/alert-review-device-integration-smoke.test.mjs` | Staged in the FR pre-commit package | Commit the verifier and smoke tooling so packaging, migration, and deployed DEVICE smoke drift are checked before every release |
...
- Production smoke with real VIDEO URLs:
  `node .scripts/alert-review-device-integration-smoke.mjs --device-base-url=... --token=... --operator-user-id=... --alert-time="YYYY-MM-DDTHH:mm:ss"`
...
- Real VIDEO integration now has docker-compose defaults plus `LiveVideo` executable smoke coverage for alert-record, coverage, record-base, export, export download readiness, and resolved download URL reachability. Local live execution is still not proven because this workstation has no reachable `127.0.0.1:6000` VIDEO service and no real smoke device/time env configured.

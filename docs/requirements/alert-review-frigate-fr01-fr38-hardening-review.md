...
| FR-27 | Reproducible video export | VIDEO manifest stores file hash, source segment facts, clip params, ffmpeg command hash, persisted artifact storage references, lifecycle expiry/status, and preserves the original source hash after download audit refresh; synchronous and async VIDEO exports now return a stable manifest_url so LiveVideo can verify the persisted manifest, manifest v2 reproducibility fields, canonical source/output SHA-256 digests, HMAC signature metadata, storage lifecycle metadata, reject local/mock/file returned export media references, reject non-video download probes such as JSON/HTML responses, and carry signer key/version metadata into smoke evidence. | Prove the same path with real VIDEO service recordings in production smoke. |

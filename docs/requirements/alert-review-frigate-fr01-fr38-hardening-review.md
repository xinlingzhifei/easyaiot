...
- `LiveVideo`: `node .scripts/alert-review-video-live-smoke.mjs --alert-record-query-url=... --record-coverage-query-url=... --record-base-url=... --record-export-url=... --device-id=... --alert-time="YYYY-MM-DD HH:mm:ss" --record-drift-retention-hours=24`; requires a real VIDEO service and real recording metadata, and fails unless the recording storage drift patrol is healthy, export status exposes a reachable `download_url`, and `manifest_url` manifest v2 includes ffmpeg command hash, source segment hashes, clip params, concat order, and output hashes. Its CLI JSON prints `storageDriftSummary` with health, checked record count, issue count, and issue reason counts.

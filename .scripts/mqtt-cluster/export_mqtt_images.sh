#!/usr/bin/env bash
# 在本机（平台服务器）拉取并导出 EMQX 镜像为离线 tar，再同步至目标机 docker load。
...
EMQX_TAR="${IMAGES_DIR}/emqx-emqx-5.8.7.tar"
...
main() {
  require_docker
  print_step "准备 EMQX 离线镜像包 -> ${IMAGES_DIR}"

  ensure_image "${EMQX_IMAGE}" \
    "docker.m.daocloud.io/emqx/emqx:5.8.7" \
    "registry.cn-hangzhou.aliyuncs.com/emqx/emqx:5.8.7"

  save_image "${EMQX_IMAGE}" "${EMQX_TAR}"
